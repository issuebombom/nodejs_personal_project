...
> - [x] Access 재구현(쿠키 전달 방식으로 변경)

### 개인 프로젝트 Lv.2 상세 업데이트 내역

23.06.12 주차(Lv.2)에 완성한 프로젝트에서 추가 및 수정된 내용입니다.

- password와 passwordConfirm 검증 기능 추가
- 게시글 수정, 삭제 전 등록된 패스워드 검증 API를 미들웨어로 변경 및 적용
- 댓글 수정, 삭제 전 등록된 패스워드 검증 API를 미들웨어로 변경 및 적용
- Access Token 발행을 쿠키에 저장하는 방식으로 변경
- 토큰 발행 방식 변동에 따른 검증 방식 변경

> 💡 `문제점`  
> 현재 엑세스 토큰이 탈취될 경우에 대한 대책이 전혀 적용되어 있지 않습니다. 만료된 엑세스 토큰을 가져오면 무조건적으로 재발급해주고 있는 상황이라 이 부분에 대해서 고민을 많이 했고, 결론적으로 어떻게 해야하는지 대안을 찾고 있는 상황입니다.
>
> 주요 논점은 '탈취된 엑세스 토큰과 유저의 엑세스 토큰을 어떻게 구분할 수 있을까?' 입니다. 현재까지 생각한 대안으로는 **_로그인 시 리프레시 토큰을 발급한 뒤 유저 데이터 베이스에 저장하고, 엑세스 토큰 발행 시 입력 데이터로서 리프레시 토큰을 활용합니다. 즉 엑세스 토큰을 통해 리프레시 토큰을 확인할 수 있게 됩니다._** 이렇게 하는 이유는 재발급 시 엑세스 토큰에 들어있는 리프레시 토큰과 데이터 베이스에 저장된 리프레시 토큰이 일치하는지 보기 위함입니다.
>
> 로그인 시 데이터베이스에 저장되는 리프레시 토큰과 유저의 엑세스 토큰이 최신화를 유지하지만 만일 이전 엑세스 토큰이 탈취되었을 경우 탈취된 엑세스 토큰에 담긴 리프레시 토큰과 데이터베이스에 저장된 리프레시 토큰값이 불일치 할 것입니다. 그러므로 엑세스 토큰이 탈취된 이후 유저가 로그인을 한다면 해당 엑세스 토큰은 사용이 불가하게 됩니다.
>
> 해당 경우는 유저가 장시간 로그인을 하지 않는 계정에 대해서 탈취가 이루어 진다면 문제가 될 수 있습니다. 이 경우에는 리프레시 토큰의 만료를 기다릴 수 밖에 없습니다. 그러므로 리프레시 토큰도 그리 길게 설정해서는 안될 것 같습니다.
>
> 📌 위 방식은 Lv.3 과제에서 적용해 보도록 하겠습니다.
